...
plaid blue bags contain 1 dull indigo bag, 4 wavy black bags, 4 clear red bags.""".strip()

line_regex = re.compile(r'^(?P<container>[a-z\s]+)bags contain(?P<content>[\d,a-z\s]+)\.$')
content_regex = re.compile(r'^\s*(?P<count>\d+)\s*(?P<color>[a-z\s]+)bag[s]*$')
SHINY_GOLD = 'shiny gold'


def parse_rule(rule):
    contents_rule = {}
    line_match = line_regex.match(rule)
    container = line_match.group('container').strip()
    if line_match.group('content').strip() == 'no other bags':
        return container, None
    for item in line_match.group('content').split(','):
        item_match = content_regex.match(item)
        contents_rule[item_match.group('color').strip()] = int(item_match.group('count'))
    return container, contents_rule


def convert_rules_to_code(rules):
    all_rules = {}
    for rule in rules.split('\n'):
        container, content = parse_rule(rule.strip())
        all_rules[container] = content
    return all_rules


def find_color_rules(all_rules, colors_inside):
    shiny_gold_count = 0
    color_tree = {}
    if colors_inside is None:
        return color_tree, shiny_gold_count
    for color_inside in colors_inside:
        color_tree[color_inside], shiny_gold_count_found = find_color_rules(all_rules, all_rules[color_inside])
        if color_inside == SHINY_GOLD:
            shiny_gold_count += 1
        shiny_gold_count += shiny_gold_count_found
    return color_tree, shiny_gold_count


def walk_rules(all_rules):
    color_tree = {}
    color_with_shiny_gold = 0
    for color, colors_inside in all_rules.items():
        color_tree[color], shiny_gold_found = find_color_rules(all_rules, colors_inside)
        if shiny_gold_found > 0:  # at least 1
            color_with_shiny_gold += 1
    return color_tree, color_with_shiny_gold


def count_bags_for_color(all_rules, bag_rules, color, depth=1):
    bags_count = 1
    if not bag_rules:
        return bags_count
    subcolors_multipliers = all_rules[color]
    for color_inside, color_parsed_rules in bag_rules.items():
        bags_for_color = count_bags_for_color(all_rules, color_parsed_rules, color_inside, depth=depth + 1)
        bags_count += subcolors_multipliers[color_inside] * bags_for_color
    return bags_count


def count_bags_for_shiny_gold(all_rules, parsed_rules):
    return count_bags_for_color(all_rules, parsed_rules[SHINY_GOLD], SHINY_GOLD) - 1  # We're looking for 1 shiny gold bag and this bag must be excluded
...
dark violet bags contain no other bags."""
    ...
    rules_to_code = convert_rules_to_code(rules)
    parsed_rules, color_with_shiny_gold = walk_rules(rules_to_code)
    print(parsed_rules, color_with_shiny_gold)
    shiny_gold_all_bags = count_bags_for_shiny_gold(rules_to_code, parsed_rules)
    print(shiny_gold_all_bags)
    assert shiny_gold_all_bags == 126
...
if __name__ == '__main__':
    if 'test' in sys.argv:
        ...
        rules_to_code = convert_rules_to_code(data)
        parsed_rules, color_with_shiny_gold = walk_rules(rules_to_code)
        print(color_with_shiny_gold)
        print(count_bags_for_shiny_gold(rules_to_code, parsed_rules))
