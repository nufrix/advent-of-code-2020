...
def prepare_answers(answers):
    ...
    for line in answers.split('\n'):
        if line == '':
            prepared_answers.append(current_group_answers)
            current_group_answers = []
            continue
...
def count_unique_common_answers(all_answers):
    answers_count = 0
    for group_answers in all_answers:
        print(group_answers)
        group_common_answers = set(group_answers[0]).intersection(*(set(item) for item in group_answers[1:]))
        print(group_common_answers)
        answers_count += len(group_common_answers)
    return answers_count


if __name__ == '__main__':
    prepared_answers = prepare_answers(data)
    answers_count = 0
    for group_answers in prepared_answers:
        answers_count += count_unique_answers(group_answers)
    print(answers_count)
    print(count_unique_common_answers(prepared_answers))
