...
class Map:
    ...
    def is_outside_map(self):
        if self.position.current_position_x > self.down_right_coordinates[0] or self.position.current_position_y > \
                self.down_right_coordinates[1]:
            return True
        return False

    def mark_visited_location(self):
        map_line = self.map[self.position.current_position_y]
        self.map[self.position.current_position_y] = map_line[
                                                     :self.position.current_position_x] + self.visited_location + map_line[
                                                                                                                  self.position.current_position_x + 1:]

    def save_map_to_file(self):
        with open(f'map-3.2-{self.position.right}-{self.position.down}.txt', 'w') as f:
            f.write('\n'.join(self.map))
    ...
    def walk(self):
        ...
        return self.position_items_count


if __name__ == '__main__':
    position_movements = [(1, 1), (3, 1), (5, 1), (7, 1), (1, 2)]
    trees_hits = []
    for position_movement in position_movements:
        position = Position(right=position_movement[0], down=position_movement[1])
        world_map = Map(pattern, position)
        result = world_map.walk()
        trees_hits.append(result['#'])

    result = 1
    for item in trees_hits:
        result *= item
    print(trees_hits, result)
