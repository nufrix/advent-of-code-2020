...
def validate_byr(data):
    match = re.match(r'^(?P<year>\d{4})$', data)
    if match:
        found_year = int(match.group('year'))
        if 1920 <= found_year <= 2002:
            return True
    return False
...
def validate_eyr(data):
    ...
    if match:
        found_year = int(match.group('year'))
        if 2020 <= found_year <= 2030:
            return True
    return False


def validate_hgt(data):
    match = re.match(r'^(?P<height>\d+)(?P<unit>(cm|in))$', data)
    if match:
...
def validate_pid(data):
    match = re.match(r'^\d{9}$', data)
    if match:
        return True
    return False


REQUIRED_FIELDS = {'byr': validate_byr, 'iyr': validate_iyr, 'eyr': validate_eyr, 'hgt': validate_hgt, 'hcl': validate_hcl, 'ecl': validate_ecl, 'pid': validate_pid}  # 'cid' is optional


def prepare_passports(passports):
    prepared_passports = []
    current_passwort = []
    for line in passports.split('\n'):
        if line == '':
            prepared_passports.append(' '.join(current_passwort))
            current_passwort = []
            continue
        current_passwort.append(line)
    return prepared_passports


def validate_passport(passport):
    passport_data = {item.strip().split(':')[0]: item.strip().split(':')[1].strip() for item in re.split(r'\s', passport) if item.strip()}
    if 'cid' in passport_data:
        passport_data.pop('cid')  # optional
    if set(passport_data.keys()) == set(REQUIRED_FIELDS.keys()):
        for passport_key, passport_value in passport_data.items():
            if not REQUIRED_FIELDS[passport_key](passport_value):
                print(passport, '->', passport_key, passport_value, 'invalid')
                return False
        return True
    return False
...
if __name__ == '__main__':
    ...
    for passport in prepare_passports(passports):
